...
if [ $? -ne 0 ]; then
  docker compose -f /home/med/tp/docker-compose.yml up registry -d
  sleep 3
fi

echo "------ DEBUT DU BUILD ------"
docker build -t backend /home/med/tp/backend
echo "------ FIN DU BUILD ------"

echo "------- DEBUT DU RUN --------"
docker compose -f /home/med/tp/docker-compose.yaml up backend -d
echo "------ FIN DU RUN ---------"
